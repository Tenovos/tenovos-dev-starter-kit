...
const metadataConfigLocation = args[3]; // s3 prefix where configs files are stored
...
async function perform() {
    ...
    if (action === 'setup') {
        ...
        const lambdaPayload = {
            body: {
                requestType: 'create',
                configLocation: metadataConfigLocation,
...
async function setup() {
    const data = readFileSync('./profile-doc.json');
    ...
    const dskConfig = extIntegObj.filter((item) => item.name === featureName);
...
async function teardown() {
    ...
    if (dskConfig && dskConfig.length > 0) {
        ...
        const dskRemoveConfig = extIntegObj.filter((item) => item.name !== featureName);
